...
app.use('/api-proxy', async (req, res, next) => {
    ...
    if (req.body) { // Only log body if it exists
        ...
        // The SDK might send systemInstruction (camelCase) or system_instruction (snake_case).
        // We find the instruction object and modify the text within it.
        const instructionKey = req.body.system_instruction ? 'system_instruction' : 'systemInstruction';
        const systemInstruction = req.body[instructionKey];

        if (systemInstruction && systemInstruction.parts && Array.isArray(systemInstruction.parts) && systemInstruction.parts.length > 0 && typeof systemInstruction.parts[0].text === 'string') {
            console.log("  Found valid system instruction object. Modifying...");
            const originalText = systemInstruction.parts[0].text;
            const fullSystemPromptText = `${SYSTEM_PROMPT}\n\n${originalText}`;
            
            // Update the text within the original object structure
            req.body[instructionKey].parts[0].text = fullSystemPromptText;
            console.log("  Successfully modified system instruction.");
        }
        // --- MODIFICATION END ---
    }
    try {
